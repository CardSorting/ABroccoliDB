...
export type JsonSqlValue = null | string | number | boolean | JsonSqlValue[] | { [key: string]: JsonSqlValue };
...
export class JsonSqlError extends Error {
  ...
  constructor(
    message: string,
    code: JsonSqlError["code"] = "ERR_JSONSQL_SYNTAX",
...
interface StoredSchema extends Record<string, unknown> {
  readonly version: 1;
  readonly name: string;
  readonly primaryKey: string;
  readonly columns: readonly JsonSqlColumn[];
...
type SqlValueExpression =
  | { readonly kind: "literal"; readonly value: JsonSqlValue }
  | { readonly kind: "parameter"; readonly index: number };

type SqlExpression =
  | { readonly kind: "compare"; readonly field: string; readonly operator: "=" | "!=" | "<>" | ">" | ">=" | "<" | "<="; readonly value: SqlValueExpression }
  | { readonly kind: "null-check"; readonly field: string; readonly negated: boolean }
  | { readonly kind: "in"; readonly field: string; readonly values: readonly SqlValueExpression[]; readonly negated: boolean }
  | { readonly kind: "between"; readonly field: string; readonly lower: SqlValueExpression; readonly upper: SqlValueExpression; readonly negated: boolean }
  | { readonly kind: "like"; readonly field: string; readonly pattern: SqlValueExpression; readonly negated: boolean }
  | { readonly kind: "not"; readonly expression: SqlExpression }
  | { readonly kind: "and" | "or"; readonly left: SqlExpression; readonly right: SqlExpression };
...
interface SelectCommand {
  ...
  readonly columns: readonly ({ readonly kind: "star" } | { readonly kind: "column"; readonly name: string; readonly alias?: string })[];
...
interface UpdateCommand {
  ...
  readonly table: string;
  readonly assignments: readonly { readonly column: string; readonly value: SqlValueExpression }[];
  readonly where?: SqlExpression;
}
...
function canonicalIdentifier(value: string, offset: number): string {
  if (
...
function cloneJsonValue(value: unknown, ancestors = new Set<object>()): JsonSqlValue {
  if (value === null || typeof value === "string" || typeof value === "boolean") return value;
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new JsonSqlError("JSONSQL values must use finite numbers", "ERR_JSONSQL_BINDINGS");
    return value;
  }
  if (Array.isArray(value)) {
    if (ancestors.has(value)) throw new JsonSqlError("JSONSQL values cannot contain cycles", "ERR_JSONSQL_BINDINGS");
    ancestors.add(value);
    ...
    const clone = Array.from({ length: value.length }, (_, index) =>
      Object.prototype.hasOwnProperty.call(value, index) ? cloneJsonValue(value[index], ancestors) : null
    );
  ...
  if (isPlainObject(value)) {
    if (ancestors.has(value)) throw new JsonSqlError("JSONSQL values cannot contain cycles", "ERR_JSONSQL_BINDINGS");
    ancestors.add(value);
    const clone: Record<string, JsonSqlValue> = {};
    for (const [key, item] of Object.entries(value)) {
      if (item === undefined) throw new JsonSqlError(`JSONSQL value at ${key} is undefined`, "ERR_JSONSQL_BINDINGS");
...
function stableJson(value: JsonSqlValue): string {
...
class SqlLexer {
  ...
  tokenize(): readonly Token[] {
    while (this.index < this.source.length) {
      ...
      if (char === "-" && this.source[this.index + 1] === "-") { this.skipLineComment(); continue; }
      if (char === "/" && this.source[this.index + 1] === "*") { this.skipBlockComment(); continue; }
      if (char === "'") { this.readString(); continue; }
      if (char === '"' || char === "`" || char === "[") { this.readQuotedIdentifier(); continue; }
      if (char === "?") {
        this.tokens.push({ kind: "parameter", text: char, offset: this.index });
        this.index++;
        continue;
      }
      if (/[a-zA-Z_]/.test(char)) { this.readWord(); continue; }
  ...
  private readQuotedIdentifier(): void {
    ...
    while (this.index < this.source.length) {
      const char = this.source[this.index++];
      if (char === close) {
        if (open !== "[" && this.source[this.index] === close) { value += close; this.index++; continue; }
  ...
  private readNumber(): void {
    ...
    if (!match) throw new JsonSqlError(`Invalid number at position ${start + 1}`);
    const value = Number(match[0]);
    if (!Number.isFinite(value)) throw new JsonSqlError(`Number is out of range at position ${start + 1}`);
  ...
  private skipBlockComment(): void {
    ...
    if (end < 0) throw new JsonSqlError(`Unterminated comment at position ${start + 1}`);
    this.index = end + 2;
  }
}
...
class SqlParser {
  ...
  parse(): { readonly command: SqlCommand; readonly parameterCount: number } {
    let command: SqlCommand;
    if (this.takeWord("CREATE")) command = this.parseCreateTable();
    else if (this.takeWord("SELECT")) command = this.parseSelect();
    else if (this.takeWord("INSERT")) command = this.parseInsert();
    else if (this.takeWord("UPDATE")) command = this.parseUpdate();
    else if (this.takeWord("DELETE")) command = this.parseDelete();
    else throw new JsonSqlError("Supported statements are CREATE TABLE, SELECT, INSERT, UPDATE, and DELETE");

    this.takeSymbol(";");
    if (!this.atEnd()) throw this.syntax("Only one SQL statement is allowed per prepared statement");
    return { command, parameterCount: this.parameterCount };
  }

  private parseCreateTable(): CreateTableCommand {
    this.expectWord("TABLE");
    const ifNotExists = this.takeWord("IF") ? (this.expectWord("NOT"), this.expectWord("EXISTS"), true) : false;
    const name = this.readIdentifier();
    this.expectSymbol("(");
    const columns: JsonSqlColumn[] = [];
    ...
    do {
      if (this.takeWord("PRIMARY")) {
        ...
        this.expectSymbol(")");
        if (primaryKey) throw this.syntax("A table can have only one primary key");
        primaryKey = key;
        ...
        let defaultValue: JsonSqlValue | undefined;
        ...
        while (this.isWord("NOT") || this.isWord("NULL") || this.isWord("PRIMARY") || this.isWord("UNIQUE") || this.isWord("DEFAULT")) {
          if (this.takeWord("NOT")) {
            ...
            if (nullabilitySpecified) throw this.syntax(`Column ${columnName} declares nullability more than once`);
            ...
            this.expectWord("KEY");
            if (columnPrimaryKey) throw this.syntax(`Column ${columnName} declares PRIMARY KEY more than once`);
            columnPrimaryKey = true;
            notNull = true;
          } else if (this.takeWord("UNIQUE")) {
            if (unique) throw this.syntax(`Column ${columnName} declares UNIQUE more than once`);
            unique = true;
          } else {
            this.expectWord("DEFAULT");
            if (hasDefault) throw this.syntax(`Column ${columnName} declares DEFAULT more than once`);
        ...
        if (unique) uniqueGroups.push([columnName]);
      }
    } while (this.takeSymbol(","));

    this.expectSymbol(")");
    if (columns.length === 0) throw this.syntax("CREATE TABLE requires at least one column");
    if (!primaryKey) throw this.syntax("CREATE TABLE requires one PRIMARY KEY column");
    const columnNames = new Set<string>();
    for (const column of columns) {
      if (columnNames.has(column.name)) throw this.syntax(`Column ${column.name} is declared more than once`);
      columnNames.add(column.name);
    }
    if (!columnNames.has(primaryKey)) throw this.syntax(`Primary key column ${primaryKey} is not declared`);
    for (const column of columns) {
      if (column.primaryKey && column.name !== primaryKey) throw this.syntax("Primary key declarations do not match");
    ...
    if (keyColumn.hasDefault) throw this.syntax("Primary keys must be supplied explicitly; automatic IDs are not supported");
  ...
  private parseSelect(): SelectCommand {
    const columns: Array<{ kind: "star" } | { kind: "column"; name: string; alias?: string }> = [];
    if (this.takeSymbol("*")) columns.push({ kind: "star" });
    else {
      do {
        const name = this.readIdentifier();
  ...
  private parseInsert(): InsertCommand {
    ...
    this.expectSymbol("(");
    const values: SqlValueExpression[] = [];
    do values.push(this.readValueExpression()); while (this.takeSymbol(","));
    this.expectSymbol(")");
    if (columns.length !== values.length) throw this.syntax("INSERT column and value counts must match");
    return { kind: "insert", table, columns, values };
  }

  private parseUpdate(): UpdateCommand {
    const table = this.readIdentifier();
    this.expectWord("SET");
    const assignments: Array<{ column: string; value: SqlValueExpression }> = [];
    do {
    ...
    if (new Set(names).size !== names.length) throw this.syntax("UPDATE cannot assign a column more than once");
    const where = this.takeWord("WHERE") ? this.parseOr() : undefined;
    return { kind: "update", table, assignments, where };
  }

  private parseDelete(): DeleteCommand {
    this.expectWord("FROM");
    const table = this.readIdentifier();
    const where = this.takeWord("WHERE") ? this.parseOr() : undefined;
  ...
  private parseOr(): SqlExpression {
    ...
    while (this.takeWord("OR")) expression = { kind: "or", left: expression, right: this.parseAnd() };
    return expression;
  }

  private parseAnd(): SqlExpression {
    let expression = this.parseNot();
    while (this.takeWord("AND")) expression = { kind: "and", left: expression, right: this.parseNot() };
    return expression;
  }

  private parseNot(): SqlExpression {
    if (this.takeWord("NOT")) return { kind: "not", expression: this.parseNot() };
    return this.parsePredicate();
  }

  private parsePredicate(): SqlExpression {
    if (this.takeSymbol("(")) {
    ...
    if (this.takeWord("LIKE")) return { kind: "like", field, pattern: this.readValueExpression(), negated };
    if (negated) throw this.syntax("NOT must be followed by IN, BETWEEN, or LIKE");
    const operator = this.expectComparisonOperator();
    return { kind: "compare", field, operator, value: this.readValueExpression() };
  }

  private readValueExpression(): SqlValueExpression {
  ...
  private readStaticValue(): JsonSqlValue {
    if (this.takeSymbol("-")) {
      const token = this.peek();
      if (token.kind !== "number") throw this.syntax("A minus sign must precede a numeric literal");
      this.index++;
      return -(token.value as number);
    }
    const token = this.peek();
    if (token.kind === "string" || token.kind === "number") { this.index++; return token.value as string | number; }
    if (this.takeWord("NULL")) return null;
  ...
  private readIdentifierList(): string[] {
    const identifiers = [this.readIdentifier()];
    while (this.takeSymbol(",")) identifiers.push(this.readIdentifier());
    if (new Set(identifiers).size !== identifiers.length) throw this.syntax("Identifier lists cannot contain duplicates");
    return identifiers;
  }

  private expectIdentifier(): Token {
    const token = this.peek();
    if (token.kind !== "identifier" && token.kind !== "word") throw this.syntax("Expected an identifier");
    this.index++;
    return token;
  }

  private expectComparisonOperator(): "=" | "!=" | "<>" | ">" | ">=" | "<" | "<=" {
    const token = this.peek();
  ...
  private peek(): Token { return this.tokens[this.index] ?? this.tokens[this.tokens.length - 1]; }
  private atEnd(): boolean { return this.peek().kind === "eof"; }
  private isWord(word: string): boolean { return this.peek().kind === "word" && this.peek().text.toUpperCase() === word; }
  private takeWord(word: string): boolean { if (!this.isWord(word)) return false; this.index++; return true; }
  private expectWord(word: string): void { if (!this.takeWord(word)) throw this.syntax(`Expected ${word}`); }
  private takeSymbol(symbol: string): boolean { if (this.peek().kind !== "symbol" || this.peek().text !== symbol) return false; this.index++; return true; }
  private expectSymbol(symbol: string): void { if (!this.takeSymbol(symbol)) throw this.syntax(`Expected '${symbol}'`); }
  private syntax(message: string): JsonSqlError { return new JsonSqlError(`${message} at character ${this.peek().offset + 1}`); }
}

function normalizeColumnType(type: string, offset: number): JsonSqlColumnType {
  switch (type) {
    case "TEXT": case "VARCHAR": case "CHAR": return "TEXT";
...
function valueFor(expression: SqlValueExpression, bindings: readonly JsonSqlValue[]): JsonSqlValue {
  return expression.kind === "literal" ? expression.value : bindings[expression.index];
}

function compareSqlValues(left: JsonSqlValue | undefined, right: JsonSqlValue): number | null {
...
function sqlValuesEqual(left: JsonSqlValue | undefined, right: JsonSqlValue): TriState {
  if (left === undefined || left === null || right === null) return null;
  const leftStructured = typeof left === "object";
  const rightStructured = typeof right === "object";
  if (leftStructured && rightStructured) return stableJson(left) === stableJson(right);
  const comparison = compareSqlValues(left, right);
  return comparison === null ? null : comparison === 0;
}

function truthNot(value: TriState): TriState { return value === null ? null : !value; }
function truthAnd(left: TriState, right: TriState): TriState {
  if (left === false || right === false) return false;
  if (left === null || right === null) return null;
  return true;
}
function truthOr(left: TriState, right: TriState): TriState {
  if (left === true || right === true) return true;
  if (left === null || right === null) return null;
  return false;
}

function foldAsciiCase(char: string): string {
  const code = char.charCodeAt(0);
  return code >= 65 && code <= 90 ? String.fromCharCode(code + 32) : char;
}

interface SqlEvaluationBudget {
  likeComparisonCells: number;
}

function likeMatches(value: string, sourcePattern: string, budget: SqlEvaluationBudget): boolean {
  const work = (value.length + 1) * (sourcePattern.length + 1);
  if (!Number.isSafeInteger(work) || work > budget.likeComparisonCells) {
    throw new JsonSqlError(
      `LIKE evaluation exceeded the ${String(MAX_LIKE_COMPARISON_CELLS_PER_QUERY)}-cell per-query limit`,
      "ERR_JSONSQL_RESOURCE_LIMIT",
    );
  }
  ...
  for (const char of Array.from(sourcePattern, foldAsciiCase)) {
    if (char !== "%" || pattern.at(-1) !== "%") pattern.push(char);
  }
  ...
  for (const char of input) {
    ...
    for (let column = 1; column <= pattern.length; column++) {
      ...
      if (token === "%") current[column] = current[column - 1] || previous[column] ? 1 : 0;
      else if (token === "_" || token === char) current[column] = previous[column - 1];
    }
    previous = current;
  }
...
function evaluateExpression(
  expression: SqlExpression | undefined,
  row: Record<string, JsonSqlValue>,
  bindings: readonly JsonSqlValue[],
  budget: SqlEvaluationBudget,
): TriState {
  ...
  switch (expression.kind) {
    ...
    case "in": {
      ...
      for (const item of expression.values) {
        ...
        if (candidate === null) foundNull = true;
        else if (typeof value === "object" && typeof candidate === "object") {
          if (stableJson(value) === stableJson(candidate)) { found = true; break; }
        } else if (compareSqlValues(value, candidate) === 0) {
          found = true;
          break;
        }
      }
      const result: TriState = found ? true : foundNull ? null : false;
    ...
    case "between": {
      ...
      const result: TriState = low === null || high === null ? null : low >= 0 && high <= 0;
      return expression.negated ? truthNot(result) : result;
    }
    case "like": {
      const value = row[expression.field];
...
function compareSqlSortValues(left: JsonSqlValue, right: JsonSqlValue): number {
  const rank = (value: JsonSqlValue): number => {
...
function valueHasType(value: JsonSqlValue, type: JsonSqlColumnType): boolean {
  switch (type) {
    case "TEXT": return typeof value === "string";
    case "INTEGER": return typeof value === "number" && Number.isSafeInteger(value);
    case "REAL": return typeof value === "number" && Number.isFinite(value);
    case "BOOLEAN": return typeof value === "boolean";
    case "JSON": case "ANY": return true;
  }
}

function tableKey(value: JsonSqlValue): string {
...
export class JsonSqlStatement {
  constructor(
    private readonly connection: JsonSqlConnection,
    private readonly command: SqlCommand,
    private readonly parameterCount: number,
    readonly sql: string,
  ) {}

  get kind(): JsonSqlStatementKind { return this.command.kind; }

  all<T extends Record<string, unknown> = Record<string, unknown>>(...parameters: readonly unknown[]): readonly T[] {
    if (this.command.kind !== "select") throw new JsonSqlError("all() is only available for SELECT statements");
    return this.connection.executeSelect(this.command, this.bind(parameters)) as readonly T[];
  }

  get<T extends Record<string, unknown> = Record<string, unknown>>(...parameters: readonly unknown[]): T | undefined {
    return this.all<T>(...parameters)[0];
  }

  run(...parameters: readonly unknown[]): JsonSqlRunResult {
    const bindings = this.bind(parameters);
    if (this.command.kind === "select") throw new JsonSqlError("run() cannot execute a SELECT statement");
    return this.connection.executeMutation(this.command, bindings);
  }

  private bind(parameters: readonly unknown[]): readonly JsonSqlValue[] {
    if (parameters.length !== this.parameterCount) {
      throw new JsonSqlError(
        `Expected ${this.parameterCount} SQL binding${this.parameterCount === 1 ? "" : "s"}, received ${parameters.length}`,
        "ERR_JSONSQL_BINDINGS",
      );
    }
 ...
 */
export class JsonSqlConnection implements JsonSqlDatabase {
  private readonly schemas = new Map<string, JsonSqlSchema>();
  private catalog: IDbTable<StoredSchema> | undefined;

  constructor(private readonly host: SqlHost) {}
  ...
  restoreSchemas(): void {
    ...
    for (const { id, record } of this.getCatalog().getAllEntries()) {
      ...
      if (schema.name !== id) throw new JsonSqlError("JSONSQL schema catalog key does not match its table name", "ERR_JSONSQL_SCHEMA");
  ...
  preserveSchemasForPostCheckpointTables(checkpointTableNames: ReadonlySet<string>): () => void {
    ...
    return () => {
      ...
      for (const { id, record } of retained) {
        if (!catalog.get(id)) catalog.put(id, record);
      }
      this.restoreSchemas();
    };
  }
  ...
  clearConstraintsForCheckpointedTables(checkpointTableNames: ReadonlySet<string>): void {
    for (const name of this.schemas.keys()) {
      if (!checkpointTableNames.has(name)) continue;
      const table = this.host.getTable<Record<string, unknown>>(name);
      if (table instanceof BroccoliDbTable) table.setConstraints(undefined);
    }
  }
  ...
  executeSelect(command: SelectCommand, bindings: readonly JsonSqlValue[]): readonly Record<string, JsonSqlValue>[] {
    ...
    for (const column of command.columns) if (column.kind === "column") this.requireColumn(schema, column.name);
    for (const order of command.orderBy) {
      const aliased = command.columns.some((column) => column.kind === "column" && column.alias === order.field);
      if (!aliased) this.requireColumn(schema, order.field);
    }
    const table = this.host.getTable<Record<string, unknown>>(schema.name);
    ...
    if (command.columns.some((column) => column.kind === "star")) return rows.map((row) => cloneJsonValue(row) as Record<string, JsonSqlValue>);
    return rows.map((row) => {
      const projected: Record<string, JsonSqlValue> = {};
  ...
  executeMutation(command: Exclude<SqlCommand, SelectCommand>, bindings: readonly JsonSqlValue[]): JsonSqlRunResult {
    if (command.kind === "create-table") return this.createTable(command);
    const schema = this.requireSchema(command.table);
    if (command.kind !== "insert") this.validateExpression(schema, command.where);
    const table = this.host.getTable<Record<string, unknown>>(schema.name);

    if (command.kind === "insert") {
      for (const column of command.columns) this.requireColumn(schema, column);
      const input: Record<string, JsonSqlValue> = {};
      ...
      if (table.get(id) !== undefined) this.constraint(`Duplicate primary key value for ${schema.name}.${schema.primaryKey}`);
      ...
      table.put(id, row);
      return { changes: 1, lastInsertRowid: row[schema.primaryKey] as string | number };
    }

    if (command.kind === "update") {
      for (const assignment of command.assignments) this.requireColumn(schema, assignment.column);
      ...
      const updated = new Map<string, Record<string, JsonSqlValue>>();
      for (const { id, row } of changed) {
        const next: Record<string, JsonSqlValue> = { ...row };
    ...
    const entries = table.getAllEntries();
    const budget = { likeComparisonCells: MAX_LIKE_COMPARISON_CELLS_PER_QUERY };
    const matches = entries.filter(({ id, record }) =>
      evaluateExpression(command.where, this.readStoredRow(schema, id, record), bindings, budget) === true);
    return { changes: table.deleteMany(matches.map(({ id }) => id)) };
  }

  private createTable(command: CreateTableCommand): JsonSqlRunResult {
    const existing = this.schemas.get(command.schema.name) ?? this.readCatalogSchema(command.schema.name);
    if (existing) {
      if (command.ifNotExists) return { changes: 0 };
      this.constraint(`Table ${command.schema.name} already exists`);
    }
    const table = this.host.getTable<Record<string, unknown>>(command.schema.name);
    ...
    this.getCatalog().put(command.schema.name, command.schema as unknown as StoredSchema);
    this.installSchema(command.schema, false);
    return { changes: 1 };
  }

  private installSchema(schema: JsonSqlSchema, restoring: boolean): void {
    const table = this.host.getTable<Record<string, unknown>>(schema.name);
    ...
    if (table instanceof BroccoliDbTable) {
      table.setConstraints({
        ...
        conflict: (constraint) => new JsonSqlError(
          `UNIQUE constraint failed: ${schema.name}.${constraint.fields.join(", ")}`,
          "ERR_JSONSQL_CONSTRAINT",
        ),
      });
    }
    this.schemas.set(schema.name, schema);
    if (restoring && !this.catalog) this.catalog = this.host.getTable<StoredSchema>(CATALOG_TABLE);
  }

  private requireSchema(name: string): JsonSqlSchema {
    const schema = this.schemas.get(name) ?? this.readCatalogSchema(name);
    if (!schema) throw new JsonSqlError(`Table ${name} has no JSONSQL schema; create it with CREATE TABLE`, "ERR_JSONSQL_SCHEMA");
    return schema;
  }

  private readCatalogSchema(name: string): JsonSqlSchema | undefined {
    const stored = this.getCatalog().get(name);
    if (!stored) return undefined;
    const schema = parseStoredSchema(stored);
    if (schema.name !== name) throw new JsonSqlError("JSONSQL schema catalog key does not match its table name", "ERR_JSONSQL_SCHEMA");
    this.installSchema(schema, false);
    return schema;
  }

  private getCatalog(): IDbTable<StoredSchema> {
    this.catalog ??= this.host.getTable<StoredSchema>(CATALOG_TABLE);
    return this.catalog;
  }

  private requireColumn(schema: JsonSqlSchema, name: string): JsonSqlColumn {
    const column = schema.columns.find((item) => item.name === name);
    if (!column) throw new JsonSqlError(`Unknown column ${schema.name}.${name}`, "ERR_JSONSQL_SCHEMA");
    return column;
  }

  private validateExpression(schema: JsonSqlSchema, expression: SqlExpression | undefined): void {
  ...
  private readStoredRow(schema: JsonSqlSchema, id: string, input: Record<string, unknown>): Record<string, JsonSqlValue> {
    if (!isPlainObject(input)) this.constraint(`Row in ${schema.name} must be a JSON object`);
    const row: Record<string, JsonSqlValue> = {};
    const names = new Set(schema.columns.map((column) => column.name));
    for (const key of Object.keys(input)) if (!names.has(key)) this.constraint(`Unknown column ${schema.name}.${key}`);
    for (const column of schema.columns) {
      ...
      if (value === null && (column.notNull || column.primaryKey)) this.constraint(`${schema.name}.${column.name} cannot be NULL`);
      if (value !== null && !valueHasType(value, column.type)) this.constraint(`${schema.name}.${column.name} must be ${column.type}`);
      Object.defineProperty(row, column.name, { value, enumerable: true, configurable: true, writable: true });
    }
    if (tableKey(row[schema.primaryKey]) !== id) this.constraint(`Stored row key does not match ${schema.name}.${schema.primaryKey}`);
    return row;
  }

  private makeInsertedRow(schema: JsonSqlSchema, input: Record<string, JsonSqlValue>): Record<string, JsonSqlValue> {
    const row: Record<string, JsonSqlValue> = {};
    const names = new Set(schema.columns.map((column) => column.name));
    for (const key of Object.keys(input)) if (!names.has(key)) this.constraint(`Unknown column ${schema.name}.${key}`);
    for (const column of schema.columns) {
  ...
  private makeStoredRow(schema: JsonSqlSchema, input: Record<string, JsonSqlValue>): Record<string, JsonSqlValue> {
    return this.makeInsertedRow(schema, input);
  }

  private assertUnique(schema: JsonSqlSchema, rows: readonly Record<string, JsonSqlValue>[]): void {
    for (const group of schema.uniqueGroups) {
      ...
      for (const row of rows) {
        ...
        if (seen.has(key)) this.constraint(`UNIQUE constraint failed: ${schema.name}.${group.join(", ")}`);
        seen.add(key);
      }
    }
  }

  private readLimit(expression: SqlValueExpression, bindings: readonly JsonSqlValue[]): number | undefined {
  ...
  private readNonNegativeInteger(expression: SqlValueExpression, bindings: readonly JsonSqlValue[], clause: string): number {
    const value = valueFor(expression, bindings);
    if (typeof value !== "number" || !Number.isSafeInteger(value)) {
  ...
  private constraint(message: string): never { throw new JsonSqlError(message, "ERR_JSONSQL_CONSTRAINT"); }
}

function parseStoredSchema(value: unknown): JsonSqlSchema {
  ...
  const uniqueGroups = value.uniqueGroups.map((group) => {
    ...
    const fields = group.map((field) => canonicalIdentifier(field as string, 0));
